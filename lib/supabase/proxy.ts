...
export async function updateSession(request: NextRequest) {
  ...
  if (!user && request.nextUrl.pathname.startsWith('/dashboard')) return NextResponse.redirect(new URL('/auth/login', request.url))
  if (user && (request.nextUrl.pathname.startsWith('/auth/login') || request.nextUrl.pathname.startsWith('/auth/sign-up'))) return NextResponse.redirect(new URL('/', request.url))
  return response
}
